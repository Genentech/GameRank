...
This vignette describes how to perform wrapper-based feature selection using the \pkg{GameRank} package.
...
\pkg{GameRank} provides a one-stop function for that: \code{check_variables} function.
...
Sometimes variable distributions may be multi-modal. \pkg{GameRank} provides a function for this task: \code{check_multimodality}.

\pkg{GameRank} determines multi-modal variables as follows: Let \eqn{k} be the number of mixture components ranging from 1 to \eqn{kmax}. The algorithm
fits first \code{m_fits} GMM models with k components using \pgk{flexmix}. Only models that converge are retained. For each k the minimum AIC
