...
# 1) Run some basic variable checks and filter degenerate variables out ----
var_checks <- check_variables( dat, NULL, lst_vars )
# var_checks %>% arrange( nmiss_pct, entropy ) %>% View

lst_vars <- var_checks %>%
  filter( !(check_missing %in% c("Drop","Bad")) & !(check_entropy %in% c("Entropy too low"))  ) %>%
  pull( variable )

dat <- dat %>%
  dplyr::select( all_of( c( lst_keys, lst_resp, lst_vars ) ) )

# 2) Construct better variables ----
res_st <- simple_transforms( dat, lst_vars )
# res_st$transformations %>% bind_rows %>% View
res_st$transformations %>% bind_rows %>% filter( "identity"!=transform ) %>% nrow
dat_st <- res_st$data
dat_st %>% head
lst_vars <- c( lst_vars, res_st$transformations %>% bind_rows %>% filter( "identity"!=transform ) %>% pull( transformed_var ) %>% as.character )

dat_md <- check_multimodality( dat_st, NULL, lst_vars )
dat_md
