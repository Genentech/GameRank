                  ...
                  ... ) 
{
  ...
  while( length(Y) <= m ) {
    ...
    best_vars = sffs_forward( dat, resp, vars, fn_train, fn_eval, Y, maximize, ... )
    ...
    cat( sprintf( "Best forward evaluation is %1.4f \n", J ) )
    k <- k + 1
    
    removing <- TRUE
    while( removing ) {
      removing <- FALSE
      best_vars <- sffs_backward( ds, dat, resp, vars, fn_train, fn_eval, Y, J, maximize, ... )
      df_evl <- bind_rows( df_evl, best_vars[['df_evl']] %>% mutate( step = k ) )
      agg_evl <- bind_rows( agg_evl, best_vars[['agg_evl']] %>% mutate( step = k ) )
      nvars <- best_vars[['best_vars']]
      if( 0 < length(nvars) ) {
