...
gplot_multimodal_variable <- function( dat, resp, vars, mumo, variable, ... ) {
  ...
  gp1 <- ggplot() +
    geom_line( data = tb_aic_agg, mapping = aes( x=k, y=min_aic ), color = "blue" ) + 
    geom_point( data = tb_aic_raw, mapping = aes( x=k, y=aic ) )
  
  gp2 <- ggplot() +
    geom_histogram( data = dat, mapping = aes_string( x=variable, y="..density.." ), breaks = bins_ucv( dat[[variable]] ), alpha=0.5 ) +
    geom_density( data = dat, mapping = aes_string( x=variable, y="..density.." ), bw = "ucv" ) 
  
  nn <- 500
  xs <- seq( min(dat[[variable]], na.rm=TRUE), max(dat[[variable]], na.rm=TRUE), length.out = nn )
  
  for( k in 1:ncol(flexmix::parameters(flx_mod)) ) {
    ...
    gp2 <- gp2 + 
      geom_line( data = plt, mapping = aes( x=xs, y=prob ), color = 1+k )
  }
  if( !is.null(cps) ) {
    gp2 <- gp2 + 
