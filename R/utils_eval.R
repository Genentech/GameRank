...
agg_evals <- function( df_evl, var, minimize ) {
  agg <- df_evl %>%
...
eval_add_vars <- function( ds, dat, resp, vars, fn_train, fn_eval, minimize, selection, add_vars = NULL, ..., var_sep = ","  ) {
  ...
  agg_evl <- agg_evals( df_evl, "added", minimize )
  best_selections <- best_selection( agg_evl )
  
  return( list( df_evl = df_evl, agg_evl = agg_evl, best_selections = best_selections ) )
}

eval_remove_vars <- function( ds, dat, resp, vars, fn_train, fn_eval, minimize, selection, remove_vars = NULL, ..., var_sep = ","  ) {
  ...
  agg_evl <- agg_evals( df_evl, "removed", minimize )
  best_selections <- best_selection( agg_evl )
