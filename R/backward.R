...
backward_selection_formula <- function( dat, # data.frame or tibble
...
) 
{
  ...
  backward_selection( dat,
                      resp, nvars,
                      fn_train, fn_eval, ds, max_iter,
                      ... )
}

backward_selection <- function( dat, # data.frame or tibble
                                # fo,  # formula of response ~ all variables for selection
                                resp,
                                nvars,
                                # Training function
...
) 
{
  ...
  for( j in 1:ncol(splits) ) {
    try({
      ...
      re <- tibble( train_performance = dp, validation_performance = vp, split = j, round = t, removed_var = "", selection = as.character(csel) ) %>%
  ...
  # 2. WHILE variables were removed DO
  removal_log <- NULL
  while( (t < max_iter ) & (1 < length(selection)) & removed ) {
    removed <- FALSE
    ...
    for( var in selection ) {
      ...
      for( j in 1:ncol(splits) ) {
        try({
          ...
          dp <-  fn_eval( fo, dat[which(1==splits[,j]),], mo, ... )
          vp <-  fn_eval( fo, dat[which(2==splits[,j]),], mo, ... )
          re <- tibble( train_performance = dp, validation_performance = vp, split = j, round = t, removed_var = var, selection = as.character(csel) ) %>%
            mutate( bias = sqrt( (train_performance - validation_performance)^2 ) )
    ...
    aperf <- rperf %>%
      group_by( removed_var, selection, round ) %>%
    ...
    if( 0 < nrow(aperf) ) {
      cat( sprintf( "Removing variable %s with mean bias reduction %1.4f \n", 
                    aperf %>% pull( removed_var ),
                    aperf %>% pull( m_diff_bias ) ) )
      selection <- setdiff( selection, aperf %>% pull( removed_var ) )
      removal_log <- removal_log %>% bind_rows( aperf %>% mutate( t = t ) )
      removed <- TRUE
    } else {
      removed <- FALSE
    }
